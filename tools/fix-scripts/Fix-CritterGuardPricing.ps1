# Fix-CritterGuardPricing.ps1
#
# Run this from your project root (e.g. C:\EnQuoteBuild):
#   powershell -ExecutionPolicy Bypass -File .\Fix-CritterGuardPricing.ps1
#   powershell -ExecutionPolicy Bypass -File .\Fix-CritterGuardPricing.ps1 -Target sandbox
#   powershell -ExecutionPolicy Bypass -File .\Fix-CritterGuardPricing.ps1 -PatchOnly
#
# THE BUG (observed on a real quote request):
#   297 ft of critter guard wire was priced as if "297" meant 297 ROLLS
#   of a product that's actually sold in 98-ft rolls at $41.75/roll --
#   producing $12,399.75 instead of the correct 4 rolls x $41.75 = $167.00.
#   A second "Critter Guard Fastening Hardware" material line incorrectly
#   matched the SAME expensive roll-kit SKU a second time (duplicate line),
#   and a "Critter Guard Installation" service with no catalog match fell
#   back to a $265/each placeholder multiplied by 297 (again treating feet
#   as a unit count), producing an absurd $78,705 line.
#   Total subtotal shown: $91,146.50 for what should be roughly $470.
#
# THE FIX (in src/features/quoteDraftAgent/draftEngine.js):
#   1. Added unit-aware quantity conversion: when a request specifies
#      quantity in feet but the matched catalog item is sold per roll/each,
#      the engine now extracts the coverage-per-unit encoded in the
#      catalog item's own name (e.g. "...98 ft...") and computes the
#      correct number of rolls needed, rounding up.
#   2. Added a precision-based match tie-breaker so a short, specific
#      catalog name (e.g. "Critter Guard Fastener Clips") is correctly
#      preferred over a much longer name that only coincidentally shares
#      a couple of the same words (e.g. a full "Solar Panel Bird Wire ...
#      Critter Guard Roll Kit ... with Fasteners" product name) -- this is
#      what fixed the duplicate-line bug.
#   3. Added a safety cap on the miscellaneous-service/product fallback:
#      when quantity is expressed in a linear unit (ft/in) and no catalog
#      per-unit rate exists, quantity is capped to 1 (not the raw linear
#      figure) and the line is clearly flagged for manual review, instead
#      of ever multiplying a large linear quantity by a flat placeholder
#      price again.
#   4. Added a deduplication pass that merges any line items which still
#      resolve to the identical catalog product across Products/Services/
#      Additional Materials sections into a single consolidated line.
#
...
# deploys using the same safe pattern as Deploy-Production.ps1.
...
function generateAssumptions(request, { fstCount, laborHours, usedWorstCase, unmatchedItems, conversionNotes }) {
  const notes = [];
...
export function generateQuoteDraft(request) {
  ...
  const notes = generateAssumptions(request, { fstCount, laborHours, usedWorstCase, unmatchedItems, conversionNotes });
...
Write-Host "draftEngine.js has been replaced with the corrected version." -ForegroundColor Green

# --- Step 3: Quick sanity check ---
$hasCoverageFn = Select-String -Path $targetFile -Pattern "extractCoveragePerUnit" -Quiet
$hasDedup = Select-String -Path $targetFile -Pattern "function deduplicateItems" -Quiet
$hasPrecision = Select-String -Path $targetFile -Pattern "precision \* 2" -Quiet

if (-not ($hasCoverageFn -and $hasDedup -and $hasPrecision)) {
    Write-Host "WARNING: One or more expected fixes were not found in the written file. Please review manually." -ForegroundColor Red
    Write-Host "  extractCoveragePerUnit present: $hasCoverageFn" -ForegroundColor Yellow
    Write-Host "  deduplicateItems present: $hasDedup" -ForegroundColor Yellow
    Write-Host "  precision tie-break present: $hasPrecision" -ForegroundColor Yellow
    exit 1
}
Write-Host "Sanity check passed: all three fixes are present in the written file." -ForegroundColor Green

if ($PatchOnly) {
    Write-Host ""
    Write-Host "PatchOnly mode: skipping build/deploy. File is patched; run your own build/test when ready." -ForegroundColor Yellow
    exit 0
}

# --- Step 4: Check for the rogue shell-level env var that bit us before ---
...
$envBackupPath = ".\.env.local.before-critterguard-fix.bak"
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Build failed (likely a syntax error in the new draftEngine.js). Restoring .env.local." -ForegroundColor Red
...
Write-Host ""
Write-Host "Done. Re-test the Quote Draft Agent with the same critter guard request on $baseUrl" -ForegroundColor Green
Write-Host "Expect: ~4 rolls x `$41.75 = ~`$167 for the wire, a single flagged ~`$265 placeholder for" -ForegroundColor Green
Write-Host "the unmatched installation service, and one `$40 fastener-clips line -- not `$91,146.50." -ForegroundColor Green