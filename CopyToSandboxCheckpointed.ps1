...
Write-Host "=== Updating copy-to-sandbox.mjs: conservative pacing + checkpoint/resume ==="
Write-Host ""
Write-Host "The previous version still hit persistent rate limits even with"
Write-Host "adaptive back-off. This version:"
Write-Host "  - Uses a fixed, conservative 2.5 second delay between EVERY write"
Write-Host "    (both quotes and products), which is slower but far more reliable."
Write-Host "  - Saves a checkpoint file (copy-progress.json) after each successful"
Write-Host "    write. If the script stops partway through (rate limit, network"
Write-Host "    blip, you press Ctrl+C), you can just run it again and it will"
Write-Host "    SKIP everything already copied instead of starting over or"
Write-Host "    creating duplicates."
Write-Host "  - Your 140 already-copied quotes from the last run are NOT lost --"
Write-Host "    they are already in the sandbox. This version will pick up from"
Write-Host "    where a fresh checkpoint starts, and will simply add any quotes"
Write-Host "    not yet present (some minor duplication of those 140 is possible"
Write-Host "    since this is a new checkpoint file -- see note below)."
Write-Host ""
Write-Host "Safety design is UNCHANGED and re-verified: production is read-only,"
Write-Host "and a hard check aborts if source and target App IDs ever match."
Write-Host ""
...
Write-Host ""
Write-Host "=== Run it ==="
Write-Host "Set your credentials (re-use if still set this session):"
Write-Host ""
...
Write-Host '  $env:BASE44_TARGET_API_KEY = "<same key -- confirmed account-level>"'
Write-Host "  node copy-to-sandbox.mjs 2>&1 | Tee-Object -FilePath .\copy-to-sandbox-log4.txt"
Write-Host ""
Write-Host "This run will take roughly 20-30+ minutes for the full 500 quotes"
Write-Host "and 540 products at this pace -- that is expected. If it stops for"
Write-Host "any reason, just run the exact same command again; it will resume"
Write-Host "automatically using copy-progress.json and skip anything already done."
Write-Host ""
Write-Host "NOTE ON THE 140 QUOTES FROM YOUR LAST RUN:"
Write-Host "Since this is a fresh checkpoint file, this run cannot know which"
Write-Host "140 quotes already made it into the sandbox from the previous script"
Write-Host "version (which did not have checkpointing). Running this version now"
Write-Host "may create a small number of duplicate quotes for those 140. This is"
Write-Host "a TEST SANDBOX, so duplicates are low-risk -- if you want a fully"
Write-Host "clean copy, you can clear the Quote table in EnQuote-StandAlone"
Write-Host "dashboard first (Data > Quote > select all > delete), which only"
Write-Host "affects the sandbox, never production, then run this script fresh."
