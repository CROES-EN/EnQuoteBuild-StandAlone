...
export default function QuoteForm({ quote, products = [], onSave, onSaveDraft, onSaveCopy, onCancel, isLoading, isAdmin }) {
  ...
  const subtotal = formData.items.reduce((sum, item) => sum + (Number(item.total) || 0), 0) + laborCharge + mileageCharge + travelCharge;
  const discountAmount = formData.discount_type === "flat"
    ? Math.min(parseFloat(formData.discount_flat_amount) || 0, subtotal)
    : subtotal * Math.min(100, Math.max(0, parseFloat(formData.discount_percent) || 0)) / 100;
  const afterDiscount = subtotal - discountAmount;
  const taxableAmount = formData.items
    .filter(item => item.taxable !== false)
    .reduce((sum, item) => sum + item.total, 0);
  const taxableAfterDiscount = subtotal > 0 ? Math.max(0, taxableAmount - (taxableAmount / subtotal) * discountAmount) : 0;
  const federalTaxAmount = taxableAfterDiscount * ((parseFloat(formData.federal_tax_percent) || 0) / 100);
  const stateTaxAmount = taxableAfterDiscount * ((parseFloat(formData.state_tax_percent) || 0) / 100);
  const localTaxAmount = taxableAfterDiscount * ((parseFloat(formData.local_tax_percent) || 0) / 100);
      ...
      <QuoteDraftButton
        ...
        onApply={(draftData) => {
          ...
          setFormData((current) => ({
            ...current,
            ...draftData,
            items: Array.isArray(draftData.items)
              ? draftData.items
              : current.items
          }));
        }}
